...
public class Kamikaze implements Enemy {
    private final String name = "Kamikaze";
    private int Health;
    ...
    public int ATKRANGE = 8;
    int spawnX, spawnY;
    int mapX,mapY,mapDisplacementX,mapDisplacementY;
    int movementSpeed = 14;
    int distanceX,distanceY;
    boolean tookDamage = false;

    KamikazeStatus KamikazeStatus = new KamikazeStatus();
    KamikazeObserver KamikazeObserver = new KamikazeObserver();
    Action action;

    private final Tile idle_sprite = new enemyTile(Assets.kamikaze,35);///To be changed
    ...
    public Kamikaze(int x, int y, int mapX, int mapY){
        ///Coordonatele mapei initial
        this.mapX = mapX;
        this.mapY = mapY;
        ///Deplasamentul fata de pozitia mapei
    ...
    public void update(int mapPositionX,int mapPositionY) {
        ...
        KamikazeObserver.check(this); //Verifica distantele dintre inamic si player urmand sa actioneze pe baza rezultatelor
        KamikazeStatus.status(KamikazeObserver);//Statusul primeste observatorul apoi
        action = KamikazeStatus.getAction();//in functie de ceea ce se decide
    ...
    public void render(Graphics g) {
       if(Health>0)
        if(!tookDamage)
            idle_sprite.Draw(g,x,y,64,64);
        else {
            g.setColor(Color.RED);
            g.fillRect(x+10,y+10,sizeX,sizeY);
            tookDamage = false;
        }
    }
