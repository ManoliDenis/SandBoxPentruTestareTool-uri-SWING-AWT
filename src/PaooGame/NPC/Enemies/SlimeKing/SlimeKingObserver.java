...
public class SlimeKingObserver {
    ...
    public void check(Enemy e){
        ...
        for(Bullet bullet : curent_level.getBullets()) {
            distance_between_entity_and_bulletX = Math.abs(e.getXRelativetoOrigin()+150 - bullet.getX());
            distance_between_entity_and_bulletY = Math.abs(e.getYRelativetoOrigin()+150 - bullet.getY());
        ...
        isinroom = IsPresent.isPresent(45,1520,1240,2310,curent_level.getGlobalPlayerX(),curent_level.getGlobalPlayerY());
        if(isinroom)
